 ...
 *  (1) RAND_SHR3 - to generate uniform random integers in [0, 2^32]
 *  (2) RAND_UNIF - to generate uniform random floats in [0, 1]
 *  (3) RAND_NORMAL - to generate (standard) normal floats.
 *
 ...
 * RAND_NORMAL. Otherwise RAND_NORMAL will produce all zeros.
 */
#ifndef _RANDNORM_H
#define _RANDNORM_H
...
/* Global variables needed by the RAND_ macros */
...
float randnorm_nfix(void); /* Needed by RAND_NORMAL */
...
#define RAND_UNIF (.5 + (signed) RAND_SHR3*.2328306e-9)
#define RAND_NORMAL (\
